...
const Pricing = () => {
  ...
  useEffect(() => {
    const translateData = async () => {
      const langto = Cookies.get("to");
      if (data && langto && langto !== "fra") {
        const translatedBtnCom = await tr(btnCom, "fra", langto);
        const translatedYear = await tr(year, "fra", langto);
        const translatedTitre = await tr(Titre, "fra", langto);
        const translatedVoirplus = await tr(voirplus, "fra", langto);
        setBtnCom(translatedBtnCom);
        setYear(translatedYear);
        setTitre(translatedTitre);
        setVoirplus(translatedVoirplus);
      }
      const translatedItems = await Promise.all(
        data.map(async (item) => {
          let it = { ...item };
    ...
    translateData();
  }, [data, btnCom, year, Titre, voirplus]);
